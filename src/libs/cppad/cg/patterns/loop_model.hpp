...
public:
        ...
        for (size_t i = 0; i < m_; i++) {
            for (size_t it = 0; it < iterationCount_; it++) {
                ...
                if (orig != (std::numeric_limits<size_t>::max)())  // some equations are not present in all iterations
                    depOrigIndexes_[orig] =
                            LoopIndexedPosition(dependentIndexes_[i][it].tape, dependentIndexes_[i][it].original, it);
            }
        }
     ...
     */
    inline size_t getLoopId() const { return loopId_; }
     ...
     */
    inline bool isContainsAtomics() const { return containsAtoms_; }
     ...
     */
    inline const size_t getIterationCount() const { return iterationCount_; }
     ...
     *         (number of equation patterns)
     */
    inline size_t getTapeDependentCount() const { return m_; }
     ...
     */
    inline const std::vector<std::vector<LoopPosition>>& getDependentIndexes() const { return dependentIndexes_; }

    /**
     * Provides groups of equations present at the same iterations
     */
    inline const std::vector<IterEquationGroup<Base>>& getEquationsGroups() const { return equationGroups_; }
     ...
     */
    inline const std::vector<std::vector<LoopPosition>>& getIndexedIndepIndexes() const { return indexedIndepIndexes_; }

    /**
     * Provides the non-indexed independent variables ([tape variable])
     */
    inline const std::vector<LoopPosition>& getNonIndexedIndepIndexes() const { return nonIndexedIndepIndexes_; }

    /**
     * Provides the independent variables related with temporary variables of the
     * original model.
     */
    inline const std::vector<LoopPosition>& getTemporaryIndependents() const { return temporaryIndependents_; }
     ...
     */
    inline const LoopIndexedPosition& getTapeDependentIndex(size_t origI) const { return depOrigIndexes_.at(origI); }

    inline const std::map<size_t, LoopIndexedPosition>& getOriginalDependentIndexes() const { return depOrigIndexes_; }
    ...
    inline void detectIndexPatterns() {
        if (indepIndexPatterns_.size() > 0) return;  // already done
    ...
    inline const std::vector<IndexPattern*>& getDependentIndexPatterns() const { return depIndexPatterns_; }

    inline const std::vector<IndexPattern*>& getIndependentIndexPatterns() const { return indepIndexPatterns_; }
    ...
    inline bool isIndexedIndependent(size_t tapeJ) const { return tapeJ < indexedIndepIndexes_.size(); }
    ...
    inline const std::vector<std::set<size_t>>& getJacobianSparsity() const { return jacTapeSparsity_; }
    ...
    static inline void printOriginalVariableIndexes(std::ostringstream& ss, const std::vector<LoopPosition>& indexes) {
