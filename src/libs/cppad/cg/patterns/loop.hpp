...
public:
    ...
    inline IndependentOrder(const std::vector<const OperationNode<Base>*>& myOrder) : order(myOrder) {}
};
...
public:
    inline Loop(EquationPattern<Base>& eq)
        ...
        if (varId_ != nullptr) varId_->adjustSize();
    }
    ...
    inline void setLinkedDependents(const std::set<std::set<size_t>*>& newLoopRelations) {
        ...
        for (std::set<size_t>* it : newLoopRelations) eqGroups_[0].linkedDependents.push_back(*it);
    }
     ...
     * The number of iterations this loop will have
     */
    inline size_t getIterationCount() const { return iterationCount_; }

    const std::vector<std::set<size_t>>& getIterationDependents() const { return iterationDependents_; }

    inline LoopModel<Base>* getModel() const { return loopModel_; }
    ...
    void merge(Loop<Base>& other,
        ...
        g.linkedEquationsByNonIndexed.insert(og.linkedEquationsByNonIndexed.begin(),
    ...
    void mergeEqGroups(Loop<Base>& other) {
        ...
        for (size_t iter = 0; iter < iterationDependents_.size(); iter++) {
            iterationDependents_[iter].insert(other.iterationDependents_[iter].begin(),
                                              other.iterationDependents_[iter].end());
        }
    }
    ...
    void createLoopModel(const std::vector<CG<Base>>& dependents,
        ...
        if (varId_ != nullptr) varId_->fill(0);
    ...
    void generateDependentLoopIndexes(const std::map<size_t, EquationPattern<Base>*>& dep2Equation) {
        ...
        for (size_t g = 0; g < eqGroups_.size(); g++) {
            ...
            map<EquationPattern<Base>*, std::vector<size_t>>
                    freeDependents;  // ordered dependents not assign to any iteration
            for (EquationPattern<Base>* eq : equations) {
            ...
            while (itDep != dependents.end()) {
                ...
                if (iterationDependents_.size() <= i) iterationDependents_.resize(i + 1);
                set<size_t>& itDepi = iterationDependents_[i];

                if (relatedEqIterationDeps.size() <= i) relatedEqIterationDeps.resize(i + 1);
                ...
                if (pos >= 0) {
                    ...
                    for (size_t dep2 : linkedDependents[pos]) {
                        ...
                        itFreeDep2 = std::find(eq2FreeDep.cbegin(), eq2FreeDep.cend(),
                                               dep2);  // consider using lower_bound instead
                    ...
                    if (posN >= 0) {
                        ...
                        auto itFreeDep =
                                find(eqFreeDep.begin(), eqFreeDep.end(), dep);  // consider using lower_bound instead
...
private:
    void addOperationArguments2Loop(const OperationNode<Base>* op,
        ...
        for (size_t a = 0; a < eqOpIndeIndep.arg2Independents.size(); a++) {
            if (eqOpIndeIndep.arg2Independents[a].empty()) continue;
    ...
    void generateIndependentLoopIndexes() {
        CPPADCG_ASSERT_UNKNOWN(iterationCount_ >
                               0);  // number of iterations and dependent indexes must have already been determined
        ...
        for (const auto& it : indexedOpIndep.op2Arguments) {
            ...
            for (size_t argumentIndex = 0; argumentIndex < aSize; argumentIndex++) {
                ...
                if (dep2Indep.empty()) continue;  // not an indexed variable
    ...
    void createLoopTapeNModel(const std::vector<CG<Base>>& dependents,
        ...
        size_t nIndep = independentsIndexed_.size() + independentsNonIndexed_.size() + independentsTemp_.size();
        ...
        if (nIndep == 0) {
            ...
            s = 0;
            for (origJ2CloneIt = nonIndexedCloneOrder.begin(); origJ2CloneIt != nonIndexedCloneOrder.end();
                 ++origJ2CloneIt, s++) {
         ...
         ******************************************************************/
        std::vector<std::vector<size_t>> dependentOrigIndexes(
        ...
        loopModel_ = new LoopModel<Base>(funIndexed.release(), containsAtoms, iterationCount_, dependentOrigIndexes,
                                         indexedIndependents, nonIndexedIndependents, temporaryIndependents);

        loopModel_->detectIndexPatterns();
    }

    inline Argument<Base> makeGraphClones(const EquationPattern<Base>& eq, OperationNode<Base>& node) {
        ...
        if ((*varIndexed_)[node] || node.getOperationType() == CGOpCode::ArrayCreation) {
            ...
            OperationNode<Base>* cloneOp = handler_.makeNode(node.getOperationType(), node.getInfo(), cloneArgs);
    ...
    OperationNode<Base>& getIndexedIndependentClone(const OperationNode<Base>* operation, size_t argIndex) {
    ...
    public:
        inline DependentIndexSorter(const EquationGroup<Base>& g,
            ...
            : group(g), freeDependents(f), dep2Equation(d2e) {}
         ...
         */
        inline std::pair<size_t, EquationPattern<Base>*> findBestDependentForIteration(size_t dep,
    ...
    private:
        inline size_t findRelativeFreeDependentInEq(EquationPattern<Base>* eq, size_t dep) {
            ...
            for (size_t depRelPos = 0; depRelPos < eqFreeDep.size(); depRelPos++) {  // consider using lower_bound
                if (eqFreeDep[depRelPos] == dep) return depRelPos;                   // found it
            }
    ...
    struct IndexedIndepSorter {
        ...
        IndexedIndepSorter(
                const std::map<const OperationNode<Base>*, const IndependentOrder<Base>*>& clone2indexedIndep_)
            : clone2indexedIndep(clone2indexedIndep_) {}

        bool operator()(const OperationNode<Base>* node1, const OperationNode<Base>* node2) {
