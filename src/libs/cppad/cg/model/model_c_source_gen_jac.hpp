...
template <class Base>
void ModelCSourceGen<Base>::generateSparseJacobianForRevSource(bool forward, MultiThreadingType multiThreadingType) {
    // size_t m = _fun.Range();
    // size_t n = _fun.Domain();
    using namespace std;
    ...
    if (!_loopTapes.empty()) {
         ...
         */
        if (forward) {
            generateSparseJacobianWithLoopsSourceFromForRev(jacInfo, maxCompressedSize, FUNCTION_SPARSE_FORWARD_ONE,
                                                            "indep", "jcol", _nonLoopFor1Elements, _loopFor1Groups,
                                                            generateFunctionNameLoopFor1);
        } else {
            generateSparseJacobianWithLoopsSourceFromForRev(jacInfo, maxCompressedSize, FUNCTION_SPARSE_REVERSE_ONE,
                                                            "dep", "jrow", _nonLoopRev1Elements, _loopRev1Groups,
    ...
    if (!_multiThreading || multiThreadingType == MultiThreadingType::NONE) {
        _sources[functionName + ".c"] = generateSparseJacobianForRevSingleThreadSource(
                functionName, jacInfo, maxCompressedSize, functionRevFor, revForSuffix, forward);
    } else {
        _sources[functionName + ".c"] = generateSparseJacobianForRevMultiThreadSource(
                functionName, jacInfo, maxCompressedSize, functionRevFor, revForSuffix, forward, multiThreadingType);
    }
...
template <class Base>
std::string ModelCSourceGen<Base>::generateSparseJacobianForRevSingleThreadSource(
...
template <class Base>
std::string ModelCSourceGen<Base>::generateSparseJacobianForRevMultiThreadSource(
        const std::string& functionName,
        std::map<size_t, CompressedVectorInfo> jacInfo,
        size_t maxCompressedSize,
        const std::string& functionRevFor,
