...
public:
    ...
    inline HessianElement()
        : location((std::numeric_limits<size_t>::max)()), row((std::numeric_limits<size_t>::max)()), count(0) {}
};
...
template <class Base>
void ModelCSourceGen<Base>::analyseSparseHessianWithLoops(
    ...
    auto flipIndices = [&](const std::vector<set<size_t>>& groupHess, size_t tape1, size_t tape2) {
        return useSymmetry && tape1 > tape2 && groupHess[tape2].find(tape1) != groupHess[tape2].end();
    };

    auto flipIndices2 = [&](const std::vector<set<size_t>>& groupHess, size_t tape1, size_t tape2) {
    ...
    for (size_t eh = 0; eh < nnz; eh++) {
        ...
        for (LoopModel<Base>* loop : _loopTapes) {
            ...
            for (size_t g = 0; g < nEqGroups; g++) {
                ...
                for (size_t iteration = 0; iteration < iter2tapeJJ.size(); iteration++) {
                    ...
                    for (const pairss& itPairs : tapePairs) {
                        ...
                        std::vector<HessianElement>& positions =
                                loopInfo.equationGroups[g].indexedIndexedPositions[tape];
                 ...
                 */
                if (posJ2 != nullptr) {
                    const std::vector<set<size_t>>& iter2tapeJ1OrigJ2 =
                            group.getHessianIndexedNonIndexedTapeIndexes(j1, j2);
                    for (size_t iteration = 0; iteration < iter2tapeJ1OrigJ2.size(); iteration++) {
                        ...
                        for (size_t tapeJ1 : tapeJ1s) {
                            ...
                            std::vector<HessianElement>* positions;
                            if (flip) {
                                positions = &loopInfo.equationGroups[g]
                                                     .nonIndexedIndexedPositions[pairss(posJ2->tape, tapeJ1)];
                                loopInfo.equationGroups[g].evalHessSparsity[posJ2->tape].insert(tapeJ1);
                            } else {
                                positions = &loopInfo.equationGroups[g]
                                                     .indexedNonIndexedPositions[pairss(tapeJ1, posJ2->tape)];
                ...
                for (size_t g = 0; g < nEqGroups; g++) {
                    ...
                    for (size_t iteration = 0; iteration < iter2TapeJ2.size(); iteration++) {
                        ...
                        for (size_t tapeJ2 : tapeJ2s) {
                            std::vector<HessianElement>& positions =
                                    loopInfo.equationGroups[g].nonIndexedIndexedPositions[pairss(posJ1->tape, tapeJ2)];
                            positions.resize(nIter);
                            positions[iteration].location = e;
                            positions[iteration].row = j1;
                            positions[iteration].count++;
            ...
            if (posJ1 != nullptr && posJ2 != nullptr) {
                ...
                if (jInNonIndexed) loopInfo.nonIndexedNonIndexedPosition[orig] = e;
            }
                ...
                for (size_t g = 0; g < nEqGroups; g++) {
                    ...
                    for (; itz != hessRow.end(); ++itz) {
                        ...
                        if (gJacRow.find(j2) != gJacRow.end()) {
                            ...
                            if (!jInNonIndexed) {
                                jInNonIndexed = true;
                                CPPADCG_ASSERT_KNOWN(loopInfo.nonIndexedNonIndexedPosition.find(orig) ==
                    ...
                    for (size_t g = 0; g < nEqGroups; g++) {
                        ...
                        for (; itTapeJ2 != hessRow.end(); ++itTapeJ2) {
                            ...
                            if (jacZk2Row.find(j2) != jacZk2Row.end()) {  // is this check truly needed?
                                ...
                                noLoopEvalJacSparsity[nOrigEq + k2].insert(
                                        j2);  // @todo: repeated operation for each group (only one needed)
                            }
                        }
                    }
...
std::vector<CG<Base>> ModelCSourceGen<Base>::prepareSparseHessianWithLoops(CodeHandler<Base>& handler,
     ...
     * d x_j2 d x_j1
     */
    analyseSparseHessianWithLoops(lowerHessRows, lowerHessCols, lowerHessOrder, noLoopEvalJacSparsity,
                                  noLoopEvalHessSparsity, noLoopEvalHessLocations, loopHessInfo, true);
    ...
    if (_funNoLoops != nullptr) {
         ...
         */
        _funNoLoops->calculateHessian4OrignalEquations(x, w, noLoopEvalHessSparsity, noLoopEvalHessLocations, hess);
    }
    ...
    for (itLoop2Info = loopHessInfo.begin(); itLoop2Info != loopHessInfo.end(); ++itLoop2Info) {
        ...
        for (size_t g = 0; g < info.equationGroups.size(); g++) {
            ...
            hessElSize += infog.indexedIndexedPositions.size() + infog.indexedTempPositions.size() +
                          infog.nonIndexedIndexedPositions.size();
        }

        if (hessElSize == 0) continue;  // no second order information
            ...
            if (!infog.indexedTempPositions.empty()) {
                for (const auto& itEval : infog.indexedTempEvals) {
                    ...
                    if (itPos != infog.indexedTempPositions.end()) {
                        ...
                        addContribution(indexedLoopResults, hessLE,
                                        createHessianContribution(handler, positions, hessVal, *info.iterationIndexOp,
                                                                  info.ifElses));
                    }
                }
            }
        ...
        for (const auto& orig2PosIt : info.nonIndexedNonIndexedPosition) {
            ...
            for (size_t g = 0; g < info.equationGroups.size(); g++) {
                ...
                if (group.iterations.size() != nIterations) {
                    CGBase v = createHessianContribution(handler, *pattern, group.iterations, nIterations, gHessVal,
                                                         *info.iterationIndexOp, info.ifElses);
        ...
        for (size_t e : lowerHessOrder) {
            ...
            } else if (hess[e].getOperationNode() != nullptr &&
                       hess[e].getOperationNode()->getOperationType() == CGOpCode::DependentMultiAssign) {
                hess[e].getOperationNode()->getArguments().push_back(*info.loopEnd);

            } else {
                hess[e] = handler.createCG(
...
std::pair<CG<Base>, IndexPattern*> createHessianContribution(CodeHandler<Base>& handler,
    ...
    if (results.size() == 1 && locations.begin()->second.size() == positions.size()) {
        ...
        CG<Base> v = createConditionalContribution(handler, branches, positions.size() - 1, positions.size(),
...
CG<Base> createHessianContribution(CodeHandler<Base>& handler,
    ...
    if (iterations.size() == nIterations) {
         ...
         * relevant iterations
         */
        return createConditionalContribution(handler, pattern, iterations, nIterations - 1, ddfdxdx, iterationIndexOp,
                                             ifElses);
    }
}
...
inline void generateLoopForJacHes(ADFun<CG<Base>>& fun,
    ...
    if (!individualColoring) {
        ...
        sparseForJacHessian(fun, xl, vw, y, jacSparsity, jacRow, jacCol, jacFlat, hesSparsity, hesRow, hesCol,
                            vhessFlat, work);
