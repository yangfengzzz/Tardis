...
std::pair<CG<Base>, IndexPattern*> createLoopResult(CodeHandler<Base>& handler,
    ...
    if (locationsIter2Pos.size() == iterCount) {
        ...
        if (reusingIfElse) {
            ...
            // depends on the iterations indexes
            set<size_t> usedIter;
            OperationNode<Base>* cond = createIndexConditionExpressionOp<Base>(handler, iterations, usedIter,
...
void printForRevUsageFunction(
        ...
        const std::map<size_t, CompressedVectorInfo>& matrixInfo,
        void (*generateLocalFunctionName)(
                std::ostringstream& cache, const std::string& modelName, const LoopModel<Base>& loop, size_t g),
        size_t nnz,
