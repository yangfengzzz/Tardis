...
template <class Base>
std::vector<std::pair<CG<Base>, IndexPattern*>> generateReverseTwoGroupOps(
...
template <class Base>
std::pair<CG<Base>, IndexPattern*> createReverseMode2Contribution(
        CodeHandler<Base>& handler,
        HessianRowGroup<Base>& group,
...
void ModelCSourceGen<Base>::prepareSparseReverseTwoWithLoops(const std::map<size_t, std::vector<size_t>>& elements) {
    ...
    analyseSparseHessianWithLoops(hessRows, hessCols, hessOrder, noLoopEvalJacSparsity, noLoopEvalHessSparsity,
    ...
    if (_funNoLoops != nullptr) {
        ...
        dzDx = _funNoLoops->calculateJacobianHessianUsedByLoops(handler, loopHessInfo, x, yNL, noLoopEvalJacSparsity,
                                                                hasAtomics);
        ...
        for (size_t g = 0; g < loopGroups.size(); g++) {
            ...
            if (createsLoop) {
                ...
                if (IndexPattern::isConstant(*indexLocalItCountPattern.get())) {
                    ...
                    itCountAssignOp = handler.makeIndexAssignNode(indexLocalItCountDcl, *indexLocalItCountPattern.get(),
                                                                  jrowIndexOp);
            ...
            auto* iterationIndexPatternOp =
                    handler.makeIndexAssignNode(indexIterationDcl, *itPattern.get(), &jrowIndexOp, localIterIndexOp);
            ...
            indexedLoopResults =
                    generateReverseTwoGroupOps(handler, lModel, info, group, tx1, dzDx, jrow2CompressedLoc);
    ...
    _sources[functionRev2 + ".c"] =
            generateGlobalForRevWithLoopsFunctionSource(elements, _loopRev2Groups, _nonLoopRev2Elements, functionRev2,
                                                        _name, _baseTypeName, "indep", generateFunctionNameLoopRev2);
    /**
     * Sparsity
     */
    _cache.str("");
...
void generateHessianRowGroups(const LoopModel<Base>& lModel,
    ...
    map<HessianTermContrib<Base>, set<size_t>> contrib2jrows =
            groupHessianRowsByContrib(info, n, indexedIndexed2jrow2Iter, indexedNonIndexed2jrow2Iter,
                                      indexedTemp2jrow2Iter, nonIndexedIndexed2jrow2Iter, tempIndexed2jrow2Iter);
    ...
    for (const auto& itC : contrib2jrows) {
         ...
         * create subgroups
         */
        subgroupHessianRowsByContrib(info, c, jrows, indexedIndexed2jrow2Iter, indexedNonIndexed2jrow2Iter,
                                     indexedTemp2jrow2Iter, nonIndexedIndexed2jrow2Iter, tempIndexed2jrow2Iter,
                                     loopGroups);
    }
}
...
std::vector<std::pair<CG<Base>, IndexPattern*>> generateReverseTwoGroupOps(
    ...
    for (size_t g = 0; g < info.equationGroups.size(); g++) {
        ...
        for (const pairss& it : group.indexedIndexed) {
            ...
            if (itPos != infog.indexedIndexedPositions.end()) {
                ...
                addContribution(
                        indexedLoopResults, hessLE,
                        createReverseMode2Contribution(handler, group, positions, infog.hess.at(tapeJ1).at(tapeJ2), tx1,
                                                       iterationIndexOp, jrow2CompressedLoc));
            }
        }
        ...
        for (const pairss& it : group.indexedTemp) {
            ...
            if (itPos != infog.indexedTempPositions.end()) {
                ...
                addContribution(indexedLoopResults, hessLE,
                                createReverseMode2Contribution(handler, group, positions, val, tx1, iterationIndexOp,
                                                               jrow2CompressedLoc));
            }
        }
    ...
    for (const pairss& orig : group.nonIndexedNonIndexed) {
        ...
        for (size_t g = 0; g < info.equationGroups.size(); g++) {
            ...
            set_intersection(group.iterations.begin(), group.iterations.end(), eqGroup.iterations.begin(),
                             eqGroup.iterations.end(), std::inserter(iterations, iterations.begin()));
            ...
            if (iterations.size() != group.iterations.size()) {
                CGBase v = createReverseMode2Contribution(handler, group, *pattern, iterations, gHessVal,
                                                          *info.iterationIndexOp, group.ifElses);
...
struct Reverse2Jrow2Iter {
    ...
    inline Reverse2Jrow2Iter(size_t row, const std::set<size_t>& iters) : jrow(row), iterations(iters) {}
};
 ...
 */
template <class Base>
inline std::map<HessianTermContrib<Base>, std::set<size_t>> groupHessianRowsByContrib(
        const loops::HessianWithLoopsInfo<Base>& info,
        size_t n,
    ...
    for (size_t j = 0; j < n; j++) {
        if (!jrows[j].empty()) contrib2jrows[jrows[j]].insert(j);
    }
 ...
 */
template <class Base>
inline void subgroupHessianRowsByContrib(
        const HessianWithLoopsInfo<Base>& info,
    ...
    if (!c.nonIndexedNonIndexed.empty()) {
        set<size_t> allIters;
        size_t nIterations = info.model->getIterationCount();
        for (size_t iter = 0; iter < nIterations; iter++) allIters.insert(allIters.end(), iter);
...
std::pair<CG<Base>, IndexPattern*> createReverseMode2Contribution(
    ...
    for (const auto& countIt : locations) {
        ...
        CG<Base> val = ddfdxdx;
        for (size_t c = 1; c < count; c++) val += ddfdxdx;
    ...
    if (results.size() == 1 && locations.begin()->second.size() == group.iterations.size()) {
        ...
        CG<Base> v = createConditionalContribution(handler, branches, positions.size() - 1, group.iterations.size(),
...
CG<Base> createReverseMode2Contribution(CodeHandler<Base>& handler,
    ...
    if (iterations.size() == group.iterations.size()) {
         ...
         * relevant iterations
         */
        return createConditionalContribution(handler, pattern, iterations, *group.iterations.rbegin(), ddfdxdx,
...
public:
    inline bool empty() const {
        return indexedIndexed.empty() && indexedNonIndexed.empty() && indexedTemp.empty() &&
               nonIndexedIndexed.empty() && nonIndexedNonIndexed.empty() && tempIndexed.empty();
    }

    inline size_t size() const {
        return indexedIndexed.size() + indexedNonIndexed.size() + indexedTemp.size() + nonIndexedIndexed.size() +
               nonIndexedNonIndexed.size() + tempIndexed.size();
    }
};
...
public:
    inline HessianRowGroup(const HessianTermContrib<Base>& c, const Reverse2Jrow2Iter& jrow2Iters)
        : HessianTermContrib<Base>(c), iterations(jrow2Iters.iterations) {
...
void ModelCSourceGen<Base>::generateFunctionNameLoopRev2(std::ostringstream& cache,
                                                         ...
                                                         const LoopModel<Base>& loop,
                                                         size_t g) {
    cache << modelName << "_" << FUNCTION_SPARSE_REVERSE_TWO << "_loop" << loop.getLoopId() << "_g" << g;
}
