...
void ModelCSourceGen<Base>::prepareSparseForwardOneWithLoops(const std::map<size_t, std::vector<size_t>>& elements) {
    ...
    if (_funNoLoops != nullptr) {
        ...
        map<size_t, map<size_t, CGBase>> dydxT =
                generateLoopFor1Jac(fun, _funNoLoops->getJacobianSparsity(), noLoopEvalSparsity, x, hasAtomics);
    ...
    for (itl2Eq = loopEqInfo.begin(); itl2Eq != loopEqInfo.end(); ++itl2Eq) {
        ...
        bool hasAtomics = isAtomicsUsed();  // TODO: improve this by checking only the current fun
        map<size_t, map<size_t, CGBase>> dyiDxtapeT =
                generateLoopFor1Jac(fun, lModel.getJacobianSparsity(), loopsEvalSparsities[&lModel], xl, hasAtomics);
        ...
        for (size_t g = 0; g < loopGroups.size(); g++) {
            ...
            auto* iterationIndexPatternOp =
                    handler.makeIndexAssignNode(indexIterationDcl, *itPattern.get(), &jcolIndexOp, localIterIndexOp);
            ...
            indexedLoopResults = generateForwardOneGroupOps(handler, lModel, info, group, iterationIndexOp, dx,
                                                            dyiDxtapeT, dzDx, jcol2CompressedLoc);
    ...
    _sources[functionFor1 + ".c"] =
            generateGlobalForRevWithLoopsFunctionSource(elements, _loopFor1Groups, _nonLoopFor1Elements, functionFor1,
                                                        _name, _baseTypeName, "indep", generateFunctionNameLoopFor1);
    /**
     * Sparsity
     */
    _cache.str("");
...
struct Forward1Jcol2Iter {
    ...
    inline Forward1Jcol2Iter(size_t col, const std::set<size_t>& iters) : jcol(col), iterations(iters) {}
};
...
public:
    inline bool empty() const { return indexed.empty() && nonIndexed.empty(); }

    inline size_t size() const { return indexed.size() + nonIndexed.size(); }
};
...
public:
    inline JacobianColGroup(const JacobianTermContrib<Base>& c, const Forward1Jcol2Iter& jcol2Iters)
        : JacobianTermContrib<Base>(c), iterations(jcol2Iters.iterations) {
...
void generateForOneColumnGroups(const LoopModel<Base>& lModel,
    ...
    map<JacobianTermContrib<Base>, set<size_t>> contrib2jcols =
            groupForOneByContrib(lModel, loopEqInfo, n, indexed2jcol2Iter, nonIndexed2Iter);
    ...
    for (const auto& itC : contrib2jcols) {
         ...
         * create subgroups
         */
        subgroupForOneByContrib(loopEqInfo, c, jcols, indexed2jcol2Iter, nonIndexed2Iter, loopGroups, c2subgroups);
    }
}
 ...
 */
template <class Base>
std::map<JacobianTermContrib<Base>, std::set<size_t>> groupForOneByContrib(
    ...
    for (size_t j = 0; j < n; j++) {
        if (!jcols[j].empty()) contrib2jcols[jcols[j]].insert(j);
    }
...
template <class Base>
std::vector<std::pair<CG<Base>, IndexPattern*>> generateForwardOneGroupOps(
        CodeHandler<Base>& handler,
        const LoopModel<Base>& lModel,
        ...
        for (size_t tapeJ : group.indexed) {
            ...
            if (itPos != jlrw.indexedPositions.end()) {
                ...
                for (size_t iter : group.iterations) {
                    if (positions[iter] !=
                        (std::numeric_limits<size_t>::max)()) {  // the element must have been requested
                        CPPADCG_ASSERT_UNKNOWN(
                                tapeJPos[iter].original !=
                                (std::numeric_limits<size_t>::max)());  // the equation must exist for this iteration
                ...
                if (!iter2jcols.empty()) {
                    ...
                    indexedLoopResults[jacLE++] = createForwardOneElement(handler, group, positions, iter2jcols, val,
                                                                          iterationIndexOp, jcol2CompressedLoc);
                }
            }
        }
        ...
        for (size_t j : group.nonIndexed) {
            ...
            for (size_t iter : group.iterations) {
                if (positions[iter] != (std::numeric_limits<size_t>::max)()) {  // the element must have been requested
                    CPPADCG_ASSERT_UNKNOWN(
                            lModel.getDependentIndexes()[tapeI][iter].original !=
                            (std::numeric_limits<size_t>::max)());  // the equation must exist for this iteration
                    iter2jcols[iter] = j;
                }
            }
...
std::pair<CG<Base>, IndexPattern*> createForwardOneElement(CodeHandler<Base>& handler,
    ...
    size_t assignOrAdd = 1;
    return createLoopResult(handler, locationsIter2Pos, positions.size(), dfdx, pattern, assignOrAdd, iterationIndexOp,
                            group.ifElses);
}
...
template <class Base>
std::map<size_t, std::map<size_t, CG<Base>>> ModelCSourceGen<Base>::generateLoopFor1Jac(
        ADFun<CGBase>& fun,
    ...
    if (!constainsAtomics) {
        ...
        if (row.size() == 0) return dyDxT;  // nothing to do
...
void ModelCSourceGen<Base>::generateFunctionNameLoopFor1(std::ostringstream& cache,
                                                         ...
                                                         const LoopModel<Base>& loop,
                                                         size_t g) {
    cache << modelName << "_" << FUNCTION_SPARSE_FORWARD_ONE << "_loop" << loop.getLoopId() << "_g" << g;
}
