...
public:
    GenericModel() : _atomic(nullptr), _evalAtomicForwardOne4CppAD(true) {}

    inline GenericModel(GenericModel&& other) noexcept
        : _atomic(other._atomic), _evalAtomicForwardOne4CppAD(other._evalAtomicForwardOne4CppAD) {
        other._atomic = nullptr;
    }

    inline virtual ~GenericModel() { delete _atomic; }
    ...
    inline bool isAtomicEvalForwardOne4CppAD() const { return _evalAtomicForwardOne4CppAD; }
    ...
    inline VectorBase ForwardZero(const VectorBase& x) {
        ...
        this->ForwardZero(ArrayView<const Base>(&x[0], x.size()), ArrayView<Base>(&dep[0], dep.size()));
        return dep;
    }
     ...
     * behavior).
     *
     * @param vx If it is not empty then it provides which independent variables are considered as variables (not
     * parameters)
    ...
    inline VectorBase Jacobian(const VectorBase& x) {
        ...
        Jacobian(ArrayView<const Base>(&x[0], x.size()), ArrayView<Base>(&jac[0], jac.size()));
        return jac;
    }
     ...
     */
    template <typename VectorBase>
    inline VectorBase Hessian(const VectorBase& x, const VectorBase& w) {
        VectorBase hess(Domain() * Domain());
        this->Hessian(ArrayView<const Base>(&x[0], x.size()), ArrayView<const Base>(&w[0], w.size()),
     ...
     */
    template <typename VectorBase>
    inline void Hessian(const VectorBase& x, const VectorBase& w, VectorBase& hess) {
     ...
     */
    template <typename VectorBase>
    inline VectorBase Hessian(const VectorBase& x, size_t i) {
    ...
    inline VectorBase ForwardOne(const VectorBase& tx) {
        ...
        this->ForwardOne(ArrayView<const Base>(&tx[0], tx.size()), ArrayView<Base>(&ty[0], ty.size()));
     ...
     * @param ty1
     */
    virtual void ForwardOne(
            ArrayView<const Base> x, size_t tx1Nnz, const size_t idx[], const Base tx1[], ArrayView<Base> ty1) = 0;
     ...
     */
    template <typename VectorBase>
    inline VectorBase ReverseOne(const VectorBase& tx, const VectorBase& ty, const VectorBase& py) {
        const size_t k = 0;
     ...
     */
    template <typename VectorBase>
    inline void ReverseOne(const VectorBase& tx, const VectorBase& ty, VectorBase& px, const VectorBase& py) {
        this->ReverseOne(ArrayView<const Base>(&tx[0], tx.size()), ArrayView<const Base>(&ty[0], ty.size()),
                         ArrayView<Base>(&px[0], px.size()), ArrayView<const Base>(&py[0], py.size()));
    }
     ...
     */
    virtual void ReverseOne(
            ArrayView<const Base> x, ArrayView<Base> px, size_t pyNnz, const size_t idx[], const Base py[]) = 0;
     ...
     */
    template <typename VectorBase>
    inline VectorBase ReverseTwo(const VectorBase& tx, const VectorBase& ty, const VectorBase& py) {
        const size_t k = 1;
        VectorBase px((k + 1) * Domain());
     ...
     */
    template <typename VectorBase>
    inline void ReverseTwo(const VectorBase& tx, const VectorBase& ty, VectorBase& px, const VectorBase& py) {
        this->ReverseTwo(ArrayView<const Base>(&tx[0], tx.size()), ArrayView<const Base>(&ty[0], ty.size()),
                         ArrayView<Base>(&px[0], px.size()), ArrayView<const Base>(&py[0], py.size()));
    }
    ...
    inline VectorBase SparseJacobian(const VectorBase& x) {
        VectorBase jac(Range() * Domain());
        SparseJacobian(ArrayView<const Base>(&x[0], x.size()), ArrayView<Base>(&jac[0], jac.size()));
        return jac;
    }
     ...
     */
    template <typename VectorBase>
    inline void SparseJacobian(const VectorBase& x, VectorBase& jac) {
        jac.resize(Range() * Domain());
        SparseJacobian(ArrayView<const Base>(&x[0], x.size()), ArrayView<Base>(&jac[0], jac.size()));
    }
    ...
    /**
     * @copydoc GenericModel::SparseJacobian(const std::vector<Base>&, std::vector<Base>&, std::vector<size_t>&,
     * std::vector<size_t>&)
     */
     ...
     */
    template <typename VectorBase>
    inline VectorBase SparseHessian(const VectorBase& x, const VectorBase& w) {
        VectorBase hess(Domain() * Domain());
        SparseHessian(ArrayView<const Base>(&x[0], x.size()), ArrayView<const Base>(&w[0], w.size()),
                      ArrayView<Base>(&hess[0], hess.size()));
        return hess;
    }
     ...
     */
    template <typename VectorBase>
    inline void SparseHessian(const VectorBase& x, const VectorBase& w, VectorBase& hess) {
        hess.resize(Domain() * Domain());
        SparseHessian(ArrayView<const Base>(&x[0], x.size()), ArrayView<const Base>(&w[0], w.size()),
                      ArrayView<Base>(&hess[0], hess.size()));
    }
    ...
    /**
     * @copydoc GenericModel::SparseHessian(const std::vector<Base>&,const std::vector<Base>&, std::vector<Base>&,
