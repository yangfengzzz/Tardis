...
void ModelCSourceGen<Base>::generateSparseReverseTwoSources() {
    ...
    generateGlobalDirectionalFunctionSource(FUNCTION_SPARSE_REVERSE_TWO, "indep", FUNCTION_REVERSE_TWO_SPARSITY,
                                            elements);
}

template <class Base>
void ModelCSourceGen<Base>::generateSparseReverseTwoSourcesWithAtomics(
...
template <class Base>
void ModelCSourceGen<Base>::generateSparseReverseTwoSourcesNoAtomics(
        const std::map<size_t, std::vector<size_t>>& elements,
...
void ModelCSourceGen<Base>::generateReverseTwoSources() {
    ...
    _cache << "#include <stdlib.h>\n"
           ...
           << "\n"
              "\n"
              "int "
           << _name << "_" << FUNCTION_SPARSE_REVERSE_TWO << "(unsigned long pos, " << argsDcl
           << ");\n"
              "void "
           << _name << "_" << FUNCTION_REVERSE_TWO_SPARSITY
    ...
    LanguageC<Base>::printFunctionDeclaration(
            ...
            {_baseTypeName + " const tx[]", _baseTypeName + " const ty[]", _baseTypeName + " px[]",
