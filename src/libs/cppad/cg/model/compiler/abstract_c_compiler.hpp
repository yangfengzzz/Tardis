...
public:
    ...
    std::string getCompilerPath() const { return _path; }

    void setCompilerPath(const std::string& path) { _path = path; }

    const std::string& getTemporaryFolder() const override { return _tmpFolder; }

    void setTemporaryFolder(const std::string& tmpFolder) override { _tmpFolder = tmpFolder; }

    bool isSaveToDiskFirst() const override { return _saveToDiskFirst; }

    void setSaveToDiskFirst(bool saveToDiskFirst) override { _saveToDiskFirst = saveToDiskFirst; }

    const std::string& getSourcesFolder() const override { return _sourcesFolder; }

    void setSourcesFolder(const std::string& srcFolder) override { _sourcesFolder = srcFolder; }

    const std::set<std::string>& getObjectFiles() const override { return _ofiles; }

    const std::set<std::string>& getSourceFiles() const override { return _sfiles; }

    const std::vector<std::string>& getCompileFlags() const { return _compileFlags; }

    void setCompileFlags(const std::vector<std::string>& compileFlags) { _compileFlags = compileFlags; }

    void addCompileFlag(const std::string& compileFlag) { _compileFlags.push_back(compileFlag); }

    const std::vector<std::string>& getLinkFlags() const { return _linkFlags; }

    void setLinkFlags(const std::vector<std::string>& linkFlags) { _linkFlags = linkFlags; }

    void addLinkFlag(const std::string& linkFlag) { _linkFlags.push_back(linkFlag); }

    const std::vector<std::string>& getCompileLibFlags() const { return _compileLibFlags; }

    void setCompileLibFlags(const std::vector<std::string>& compileLibFlags) { _compileLibFlags = compileLibFlags; }

    void addCompileLibFlag(const std::string& compileLibFlag) { _compileLibFlags.push_back(compileLibFlag); }
    ...
    virtual void compileSources(const std::map<std::string, std::string>& sources,
        ...
        if (sources.empty()) return;  // nothing to do
        ...
        if (timer != nullptr) {
            ...
            if (timer->getMaxLineWidth() < ms) timer->setMaxLineWidth(ms);
        } else if (_verbose) {
            std::cout << std::endl;
        }
        ...
        for (it = sources.begin(); it != sources.end(); ++it) {
            ...
            if (timer != nullptr || _verbose) {
                os << "[" << std::setw(countWidth) << std::setfill(' ') << std::right << count << "/" << sources.size()
                   << "]";
            }
            ...
            if (timer != nullptr) {
                ...
                std::cout << os.str() << " compiling " << std::setw(maxsize + 9) << std::setfill('.') << std::left
                          << ("'" + file + "' ") << " ";
                os.str("");
                std::cout.flush();
    ...
    virtual ~AbstractCCompiler() { cleanup(); }
