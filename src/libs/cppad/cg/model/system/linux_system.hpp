...
inline std::string createPath(std::initializer_list<std::string> folders, const std::string& file) {
    std::string path;

    size_t n = file.size();
    for (const std::string& folder : folders) n += folder.size() + 1;
    path.reserve(n);
...
inline std::string createPath(const std::string& folder, const std::string& file) {
