...
public:
    explicit LinuxDynamicLib(std::string dynLibName, int dlOpenMode = RTLD_NOW)
        : _dynLibName(std::move(dynLibName)), _dynLibHandle(nullptr) {
        std::string path;
        if (_dynLibName[0] == '/') {
        ...
        CPPADCG_ASSERT_KNOWN(_dynLibHandle != nullptr,
                             ("Failed to dynamically load library '" + _dynLibName + "': " + dlerror()).c_str())
