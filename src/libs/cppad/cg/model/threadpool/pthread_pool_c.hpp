...
enum ElapsedTimeReference { ELAPSED_TIME_AVG, ELAPSED_TIME_MIN };
...
static int thpool_add_job(ThPool*, thpool_function_type function, void* arg, const float* avgElapsed, float* elapsed);
...
void cppadcg_thpool_add_job(thpool_function_type function, void* arg, float* avgElapsed, float* elapsed) {
...
void cppadcg_thpool_add_jobs(thpool_function_type functions[],
    ...
    if (!cppadcg_pool_disabled) {
        cppadcg_thpool_prepare();
        if (cppadcg_pool != NULL) {
            thpool_add_jobs(cppadcg_pool, functions, args, avgElapsed, elapsed, order, job2Thread, nJobs,
                            lastElapsedChanged);
            return;
        }
    }
...
static int comparePair(const void* a, const void* b) {
    if (((pair_double_int*)a)->val < ((pair_double_int*)b)->val) return -1;
    if (((pair_double_int*)a)->val == ((pair_double_int*)b)->val) return 0;
    return 1;
}

void cppadcg_thpool_update_order(
        float refElapsed[], unsigned int nTimeMeas, const float elapsed[], int order[], int nJobs) {
    if (nJobs == 0 || refElapsed == NULL || elapsed == NULL || order == NULL) return;
...
static void jobqueue_push(JobQueue* queue, Job* newjob_p);
static void jobqueue_multipush(JobQueue* queue, Job* newjob[], int nJobs);
static int jobqueue_push_static_jobs(
        ThPool* thpool, Job* newjobs[], const float avgElapsed[], int jobs2thread[], int nJobs, int lastElapsedChanged);
...
static float get_thread_time(struct timespec* cputime, int* info) {
...
static float get_monotonic_time(struct timespec* time, int* info) {
...
void timespec_diff(struct timespec* end, struct timespec* start, struct timespec* result) {
 ...
 */
static int thpool_add_job(
        ThPool* thpool, thpool_function_type function, void* arg, const float* avgElapsed, float* elapsed) {
    Job* newjob;
...
static int jobqueue_push_static_jobs(ThPool* thpool,
    ...
    if (nJobs < num_threads) num_threads = nJobs;
...
static void thpool_wait(ThPool* thpool) {
    ...
    while (thpool->jobqueue->len || thpool->jobqueue->group_front ||
           thpool->num_threads_working) {  //// PROBLEM HERE!!!! len is not locked!!!!
...
void thpool_cleanup(ThPool* thpool) {
    ...
    for (int j = 0; j < thpool->num_threads; ++j) {
        ...
        while (workGroup != NULL) {
            ...
            fprintf(stdout,
                    "# Thread %i, Group %i, started at %ld.%.9ld, ended at %ld.%.9ld, elapsed %ld.%.9ld, executed %i "
                    "jobs\n",
                    thread->id, gid, workGroup->startTime.tv_sec, workGroup->startTime.tv_nsec,
                    workGroup->endTime.tv_sec, workGroup->endTime.tv_nsec, diffTime.tv_sec, diffTime.tv_nsec,
                    workGroup->size);
            ...
            for (int i = 0; i < workGroup->size; ++i) {
                ...
                fprintf(stdout,
                        "## Thread %i, Group %i, Job %i, started at %ld.%.9ld, ended at %ld.%.9ld, elapsed %ld.%.9ld\n",
                        thread->id, gid, job->id, job->startTime.tv_sec, job->startTime.tv_nsec, job->endTime.tv_sec,
                        job->endTime.tv_nsec, diffTime.tv_sec, diffTime.tv_nsec);
            }
 ...
 */
static void jobqueue_push_internal(JobQueue* queue, Job* newjob) {
    newjob->prev = NULL;
 ...
 */
static void jobqueue_push(JobQueue* queue, Job* newjob) {
    pthread_mutex_lock(&queue->rwmutex);
...
static void jobqueue_extract_single_group(JobQueue* queue, WorkGroup* group) {
...
static WorkGroup* jobqueue_pull(ThPool* thpool, int id) {
    ...
    if (schedule_strategy == SCHED_STATIC && queue->group_front != NULL) {
        ...
        if (cppadcg_pool_verbose) {
            if (schedule_strategy == SCHED_GUIDED) {
                if (queue->len == 1)
                    ...
                    fprintf(stdout,
                            "jobqueue_pull(): Thread %i using single-job instead of multi-job (no timing "
                            "information)\n",
                            id);
        ...
        if (job->avgElapsed == NULL) {
            if (cppadcg_pool_verbose) {
                fprintf(stderr,
                        "jobqueue_pull(): Thread %i using single job instead of multi-job (No timing information for "
                        "current job)\n",
                        id);
            }
            ...
            job = job->prev;
            target_duration =
                    queue->total_time * cppadcg_pool_guided_maxgroupwork / thpool->num_threads;  // always positive
            ...
            if (duration_next > queue->highest_expected_return) queue->highest_expected_return = duration_next;
        }
    }
...
const size_t CPPADCG_PTHREAD_POOL_C_FILE_SIZE = 42259;
