...
void ModelCSourceGen<Base>::generateSparseForwardOneSources() {
    ...
    generateGlobalDirectionalFunctionSource(FUNCTION_SPARSE_FORWARD_ONE, "indep", FUNCTION_FORWARD_ONE_SPARSITY,
                                            elements);
}

template <class Base>
void ModelCSourceGen<Base>::generateSparseForwardOneSourcesWithAtomics(
...
template <class Base>
void ModelCSourceGen<Base>::generateSparseForwardOneSourcesNoAtomics(
        const std::map<size_t, std::vector<size_t>>& elements) {
    using std::vector;
...
void ModelCSourceGen<Base>::generateForwardOneSources() {
    ...
    _cache << "#include <stdlib.h>\n"
           ...
           << "\n"
              "\n"
              "int "
           << _name << "_" << FUNCTION_SPARSE_FORWARD_ONE << "(unsigned long pos, " << argsDcl
           << ");\n"
              "void "
           << _name << "_" << FUNCTION_FORWARD_ONE_SPARSITY
    ...
    LanguageC<Base>::printFunctionDeclaration(
            ...
            {_baseTypeName + " const tx[]", _baseTypeName + " ty[]", langC.generateArgumentAtomicDcl()});
    _cache << " {\n"
