 ...
 */
template <class VectorSet, class VectorSet2>
inline void multMatrixMatrixSparsityTrans(
        const VectorSet& aT, const VectorSet2& b, CppAD::vector<std::set<size_t>>& rT, size_t m, size_t n, size_t q) {
...
inline CppAD::sparse_rc<VectorSizet> toSparsityPattern(const VectorSet& inPattern, size_t m, size_t n) {
...
inline void printTripletMatrix(const VectorSize& rows, const VectorSize& cols, const VectorBase& values) {
