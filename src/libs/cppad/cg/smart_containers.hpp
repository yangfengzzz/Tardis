...
public:
    ...
    inline SmartVectorPointer(size_t size) : v(size) {}

    inline SmartVectorPointer(std::vector<Base*>& v_) { v.swap(v_); }

    inline size_t size() const { return v.size(); }

    inline bool empty() const { return v.empty(); }

    inline void reserve(size_t n) { v.reserve(n); }

    inline void push_back(Base* x) { v.push_back(x); }

    inline Base* operator[](size_t n) const { return v[n]; }

    inline Base*& operator[](size_t n) { return v[n]; }
...
public:
    ...
    inline SmartSetPointer(std::set<Base*>& s_) { s.swap(s_); }

    inline size_t size() const { return s.size(); }

    inline bool empty() const { return s.empty(); }

    inline iterator begin() const { return s.begin(); }

    inline iterator end() const { return s.end(); }

    inline std::pair<iterator, bool> insert(Base* x) { return s.insert(x); }

    inline void erase(iterator pos) { s.erase(pos); }

    inline size_t erase(Base* x) { return s.erase(x); }
...
public:
    ...
    inline SmartListPointer(const std::set<Base*>& l_) { l.swap(l_); }

    inline size_t size() const { return l.size(); }

    inline bool empty() const { return l.empty(); }

    inline void push_front(Base* x) { l.push_front(x); }

    inline void pop_front() { l.pop_front(); }

    inline void push_back(Base* x) { l.push_back(x); }

    inline void pop_back() { l.pop_back(); }
...
public:
    ...
    inline reverse_iterator rbegin() { return m.rbegin(); }

    inline const_reverse_iterator rbegin() const { return m.rbegin(); }

    inline reverse_iterator rend() { return m.rend(); }

    inline const_reverse_iterator rend() const { return m.rend(); }

    inline Value*& operator[](const Key& key) { return m[key]; }
