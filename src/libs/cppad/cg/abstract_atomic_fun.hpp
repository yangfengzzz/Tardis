...
protected:
     ...
     *                   evaluation of other forward/reverse modes.
     */
    explicit CGAbstractAtomicFun(const std::string& name, bool standAlone = false)
        : Super(name), id_(createNewAtomicFunctionID()), standAlone_(standAlone) {
...
private:
    inline bool evalForwardValues(
            size_t q, size_t p, const CppAD::vector<CGB>& tx, CppAD::vector<Base>& tyb, size_t ty_size) {
