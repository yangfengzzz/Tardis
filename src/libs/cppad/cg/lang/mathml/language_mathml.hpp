...
public:
    ...
    inline const std::string& getAssignMarkup() const { return _assignStr; }

    inline void setAssignMarkup(const std::string& assign) { _assignStr = assign; }

    inline const std::string& getAddAssignMarkup() const { return _assignAddStr; }

    inline void setAddAssignMarkup(const std::string& assignAdd) { _assignAddStr = assignAdd; }
     ...
     */
    inline const std::string& getMultiplicationMarkup() const { return _multOpStr; }
     ...
     *       multiplication markup string
     */
    inline void setMultiplicationMarkup(const std::string& multOpStr) { _multOpStr = multOpStr; }
     ...
     */
    inline const std::string& getMultiplicationConstParMarkup() const { return _multValOpStr; }
     ...
     * to distinguish.
     */
    inline void setMultiplicationConstParMarkup(const std::string& multValOpStr) { _multValOpStr = multValOpStr; }
     ...
     */
    void setJavascript(const std::string& javascript) { _javascript = javascript; }

    const std::string& getJavascript() const { return _javascript; }
     ...
     */
    void setHeadExtraMarkup(const std::string& headExtra) { _headExtra = headExtra; }

    const std::string& getHeadExtraMarkup() const { return _headExtra; }
     ...
     */
    virtual const std::string& getEquationStartMarkup() const { return _startEq; }

    /**
     * Provides the markup after each equation.
     */
    virtual const std::string& getEquationEndMarkup() const { return _endEq; }
     ...
     */
    virtual const std::string& getForStartMarkup() const { return _forStart; }

    /**
     * Provides the markup after each loop.
     */
    virtual const std::string& getForEndMarkup() const { return _forEnd; }
     ...
     */
    virtual const std::string& getIfStartMarkup() const { return _ifStart; }

    /**
     * Provides the markup after each If.
     */
    virtual const std::string& getIfEndMarkup() const { return _ifEnd; }
     ...
     */
    virtual const std::string& getElseIfStartMarkup() const { return _elseIfStart; }

    /**
     * Provides the markup after each else if.
     */
    virtual const std::string& getElseIfEndMarkup() const { return _elseIfEnd; }
     ...
     */
    virtual const std::string& getElseStartMarkup() const { return _elseStart; }

    /**
     * Provides the markup after each else.
     */
    virtual const std::string& getElseEndMarkup() const { return _elseEnd; }
    ...
    inline void setSaveVariableRelations(bool save) { _saveVariableRelations = save; }

    bool requiresVariableDependencies() const override { return _saveVariableRelations; }
    ...
    static inline void printIndexCondExpr(std::ostringstream& out,
        ...
        for (size_t e = 0; e < infoSize; e += 2) {
            ...
            if (min == max) {
                ...
                if (infoSize != 2) out << "<mfenced><mrow>";

                if (max - min == 1)
                    out << "<mn>" << min << "</mn><mo>==</mo><mi class='index'>" << index
                        << "</mi><mo>&or;</mo><mi class='index'>" << index << "</mi><mo>==</mo><mn>" << max << "</mn>";
                else
                    out << "<mn>" << min << "</mn><mo>&le;</mo><mi class='index'>" << index
                        << "</mi><mo>&and;</mo><mi class='index'>" << index << "</mi><mo>&le;</mo><mn>" << max
                        << "</mn";

                if (infoSize != 2) out << "</mrow></mfenced>";
            }
        }
    }
    ...
    static inline void linearIndexPattern2String(std::ostream& os, const LinearIndexPattern& lip, const Node& index);
...
protected:
    void generateSourceCode(std::ostream& out, std::unique_ptr<LanguageGenerationData<Base>> info) override {
            ...
            if (info->zeroDependents) {
                ...
                for (size_t i = 0; i < depArg.size(); i++) {
                    ...
                    if (a.array) {
                        ...
                        _code << "<mrow id='" << createHtmlID(*(*_dependent)[i].getOperationNode()) << "' class='dep'>"
                              << _nameGen->generateDependent(i) << "</mrow>";
                    }
                    _code << _assignStr;
        ...
        if (!dependentDuplicates.empty()) {
            ...
            for (size_t index : dependentDuplicates) {
                ...
                _code << _startEq << "<mrow id='" << createHtmlID(depNode) << "' class='dep'>" << varName << "</mrow>"
                      << _assignStr << "<mrow id='" << createHtmlID(depNode) << "' class='dep'>" << origVarName
                      << "</mrow>";
                printAssignmentEnd();
            }
        }
        ...
        for (size_t i = 0; i < dependent.size(); i++) {
            if (dependent[i].isParameter()) {
                if (!_ignoreZeroDepAssign || !dependent[i].isIdenticalZero()) {
                    ...
                    _code << _startEq << "<mrow id='" << depId << "' class='dep'>" << varName << "</mrow>"
                          << _assignStr;  // id='" << createID(??)
                ...
                _code << _startEq << "<mrow id='" << depId << "' class='dep'>" << varName << "</mrow>" << _assignStr
                      << "<mrow id='" << createHtmlID(dependent[i].getOperationNode()) << "' class='indep'>"
                      << indepName << "</mrow>";
    ...
    inline virtual void printAlgorithmFileStart(std::ostream& out) {
        out << "<!DOCTYPE html>" << _endline << "<html lang=\"en\">" << _endline << "<head>" << _endline
            << "<meta charset=\"utf-8\">" << _endline << "<title>" << _filename << "</title>" << _endline;
        ...
        if (!_style.empty()) {
            out << "<style>" << _endline << _style << _endline << "</style>" << _endline;
        }
        ...
        if (!_javascript.empty()) {
            out << "<script type=\"text/javascript\">" << _endline << _javascript << _endline << "</script>"
                << _endline;
        }

        out << "</head>" << _endline << "" << _endline << "<body>" << _endline << "<!-- source file for '" << _filename
            << "' (automatically generated by CppADCodeGen) -->" << _endline << "<div id='algorithm'>" << _endline;
    }
    ...
    inline virtual void printAlgorithmFileEnd(std::ostream& out) {
        ...
        out << "</div>" << _endline << "</body>" << _endline << "</html>";
    }
    ...
    inline virtual void printAssignmentStart(Node& node, const std::string& varName, bool isDep) {
        _code << _startEq;
        _code << "<mrow id='" << createHtmlID(node) << "' class='" << (isDep ? "dep" : "tmp") << "'>" << varName
              << "</mrow>";
    ...
    inline virtual void printAssignmentEnd() { _code << _endEq << _endline; }
     ...
     */
    inline std::string createHtmlID(const Node* var) { return createHtmlID(*var); }
    ...
    virtual void printIndependentVariableName(Node& op) {
        ...
        _code << "<mrow id='" << createHtmlID(op) << "' class='indep'>"
              << _nameGen->generateIndependent(op, getVariableID(op)) << "</mrow>";
    }
    ...
    virtual unsigned printExpression(Node& node) {
        if (getVariableID(node) > 0) {
            ...
            if (getVariableID(node) >= _minTemporaryVarID || op == CGOpCode::ArrayCreation ||
                ...
                _code << "<mrow id='" << createHtmlID(node) << "' class='tmp'>" << name
                      << "</mrow>";  // TODO!!!!!!!!!!!!!!!!!!!!!!!
    ...
    virtual void printPowFunction(Node& op) {
        ...
        _code << "<msup>";
        if (encloseBase) _code << "<mfenced>";
        _code << "<mrow>";
        print(op.getArguments()[0]);
        _code << "</mrow>";
        if (encloseBase) _code << "</mfenced>";
        if (encloseExpo) _code << "<mfenced>";
        _code << "<mrow>";
    ...
    virtual void printConditionalAssignment(Node& node) {
        ...
        if ((trueCase.getParameter() != nullptr && falseCase.getParameter() != nullptr &&
            ...
            _code << "</mo>";
            print(right);
            _code << "</mrow></mfenced>" << _endEq << _endline << _condBodyStart << _endline;
            ...
            // else
            _code << _elseStart << _startEq << "<mi>else</mi>" << _endEq << _endline << _condBodyStart << _endline;
            // checkEquationEnvStart(); // no need
            printAssignmentStart(node, varName, isDep);
            print(falseCase);
            printAssignmentEnd(node);
    ...
    virtual void printAtomicForwardOp(Node& atomicFor) {
        ...
        _code << _startEq << _info->atomicFunctionId2Name.at(id)
              << "<mo>.</mo><mo>forward</mo>"
                 "<mfenced separators=','>"
                 "<mn>"
              << q
    ...
    virtual void printIndexAssign(Node& node) {
        ...
        _code << _startEq << "<mi class='index'>" << (*inode.getIndex().getName()) << "</mi>" << _assignStr;
    ...
    virtual void printParameter(const Base& value) {
        ...
        if (pos != std::string::npos) {
            ...
            while (pos < number.size() - 1 && number[pos] == '0') pos++;  // remove zeros
