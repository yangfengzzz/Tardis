...
void LanguageMathML<Base>::printArrayCreationOp(OperationNode<Base>& array) {
    ...
    for (size_t i = 0; i < argSize; i++) {
        ...
        if (newValue) {
            if (firstElement) {
                _code << _startEq << "<mrow class='tmp'>" << auxArrayName_ << "</mrow>" << _assignStr
                      << _nameGen->generateTemporaryArray(array, getVariableID(array)) << _endEq
            ...
            if (newI == i) {
                ...
                _code << _startEq << "<mrow class='tmp'>" << auxArrayName_ << "</mrow><mfenced open='[' close=']'><mn>"
                      << i << "</mn></mfenced>" << _assignStr;
                print(args[i]);
...
void LanguageMathML<Base>::printSparseArrayCreationOp(OperationNode<Base>& array) {
    ...
    if (argSize == 0) return;  // empty array
    ...
    for (size_t i = 0; i < argSize; i++) {
        ...
        if (newValue) {
            if (firstElement) {
                _code << _startEq << "<mrow class='tmp'>" << auxArrayName_ << "</mrow>" << _assignStr
                      << _nameGen->generateTemporarySparseArray(array, getVariableID(array)) << _endEq
            ...
            if (newI == i) {
                ...
                _code << _startEq << "<mi>" << _C_SPARSE_INDEX_ARRAY << "</mi><mfenced open='[' close=']'><mrow>";
            ...
            _code << _startEq << "<mi>" << _C_SPARSE_INDEX_ARRAY << "</mi><mfenced open='[' close=']'><mrow>";
            if (startPos != 0) _code << "<mn>" << startPos << "</mn><mo>+</mo>";
            _code << "<mn>" << i << "</mn></mrow></mfenced>" << _assignStr << "<mn>" << info[i + 1] << "</mn>" << _endEq
                  << _endline;
        }
    }
}
...
inline size_t LanguageMathML<Base>::printArrayCreationUsingLoop(size_t startPos,
    ...
    if (ref.getOperation() != nullptr) {
        ...
        if (refOp.getOperationType() == CGOpCode::Inv) {
             ...
             */
            for (; i < argSize; i++) {
                if (isSameArgument(args[i], tmpArrayValues[startPos + i])) break;  // no assignment needed

                if (args[i].getOperation() == nullptr || args[i].getOperation()->getOperationType() != CGOpCode::Inv ||
                    !_nameGen->isConsecutiveInIndepArray(
                            *args[i - 1].getOperation(), getVariableID(*args[i - 1].getOperation()),
            ...
            if (offset == 0)
                arrayAssign << "<mrow class='indep'>" << indep << "</mrow>"
                            << "<mfenced open='[' close=']'><mi>i</mi></mfenced>";
            else
                arrayAssign << "<mrow class='indep'>" << indep << "</mrow>" << "<mfenced open='[' close=']'><mrow><mn>"
                            << offset << "</mn> <mo>+</mo> <mi>i</mi></mrow></mfenced>";
            ...
            for (; i < argSize; i++) {
                ...
                if (!_nameGen->isInSameIndependentArray(refOp, getVariableID(refOp), *args[i].getOperation(),
            ...
            Plane2DIndexPattern p2dip(lip2, new LinearIndexPattern(0, 1, 1, 0));

            std::unique_ptr<OperationNode<Base>> op2(OperationNode<Base>::makeTemporaryNode(
                    CGOpCode::LoopIndexedIndep, refOp.getInfo(), refOp.getArguments()));
            op2->getInfo()[1] = (std::numeric_limits<size_t>::max)();  // just to be safe (this would be the index
                                                                       // pattern id in the handler)
    ...
    _indentationLevel++;
    _code << _startEq << "<mrow class='tmp'>" << auxArrayName_
          << "</mrow><mfenced open='[' close=']'><mi>i</mi></mfenced>" << _assignStr << arrayAssign.str() << _endEq
          << _endline;
    _indentationLevel--;
...
void LanguageMathML<Base>::printArrayElementOp(OperationNode<Base>& op) {
    ...
    _code << "<mo>(</mo><mrow class='tmp'>" << arrayName << "</mrow><mo>)</mo><mfenced open='[' close=']'><mn>"
          << op.getInfo()[0] << "</mn></mfenced>";
}
...
inline void LanguageMathML<Base>::printArrayStructInit(const std::string& dataArrayName,
                                                       ...
                                                       size_t k) {
    _ss.str("");
    _ss << "<mrow class='tmp'>" << dataArrayName << "</mrow><mfenced open='[' close=']'><mn>" << pos
        << "</mn></mfenced>";
    printArrayStructInit(_ss.str(), *arrays[k]);
}

template <class Base>
inline void LanguageMathML<Base>::printArrayStructInit(const std::string& dataArrayName, OperationNode<Base>& array) {
    /**
     * TODO: finish this
     */
    ...
    if (array.getOperationType() == CGOpCode::ArrayCreation) {
        ...
        _code << dataArrayName << "<mo>.</mo><mi>size</mi><mo>=</mo><mn>" << size << "</mn>" << dataArrayName
              << "<mo>.</mo><mi>sparse</mi><mo>=</mo><mn>" << false << "</mn>";
    } else {
        // sparse array
        ...
        _code << dataArrayName << "<mo>.</mo><mi>size</mi><mo>=</mo><mn>" << array.getInfo()[0] << "</mn>"
              << dataArrayName << "<mo>.</mo><mi>sparse</mi><mo>=</mo><mn>" << true << "</mn>" << dataArrayName
              << "<mo>.</mo><mi>nnz</mi><mo>=</mo><mn>" << nnz << "</mn>";
        if (nnz > 0) {
            size_t id = getVariableID(array);
            _code << dataArrayName << "<mo>.</mo><mi>idx</mi><mo>=</mo>&amp;<mo>(</mo><mi>" << _C_SPARSE_INDEX_ARRAY
                  << "</mi><mfenced open='[' close=']'><mn>" << (id - 1) << "</mn></mfenced><mo>)</mo>";
        }
    }
    _code << _endline;
}
