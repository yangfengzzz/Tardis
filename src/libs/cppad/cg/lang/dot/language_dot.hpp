...
public:
    ...
    inline void setFilename(const std::string& name) { _filename = name; }
     ...
     */
    inline void setIndepNodeStyle(const std::string& indepNodeStyle) { _indepNodeStyle = indepNodeStyle; }

    /**
     * Provides the style for the independent variable nodes
     */
    inline const std::string& getIndepNodeStyle() const { return _indepNodeStyle; }

    /**
     * Defines the style for the dependent variable nodes
     */
    inline void setDepNodeStyle(const std::string& depNodeStyle) { _depNodeStyle = depNodeStyle; }

    /**
     * Provides the style for the dependent variable nodes
     */
    inline const std::string& getDepNodeStyle() const { return _depNodeStyle; }

    /**
     *
     */
    inline void setCombineParameterNodes(bool combineParameterNodes) { _combineParameterNodes = combineParameterNodes; }

    /**
     *
     */
    inline bool isCombineParameterNodes() const { return _combineParameterNodes; }
...
protected:
    ...
    inline size_t getVariableID(const OperationNode<Base>& node) const { return _info->varId[node]; }
    ...
    inline void printEdges(const std::string& name, const OperationNode<Base>& node, const std::string& style = "") {
    ...
    inline void printEdge(const OperationNode<Base>& from, const std::string& to, const std::string& style = "") {
        _code << makeNodeName(from);
        _code << " -> " << to;
        if (!style.empty()) _code << "[" << style << "]";
    }

    inline void printEdge(const std::string& from, const std::string& to, const std::string& style = "") {
        _code << from << " -> " << to;
        if (!style.empty()) _code << "[" << style << "]";
    }
    ...
    virtual std::string printPowFunction(OperationNode<Base>& op) {
        ...
        printEdges(name, op, std::vector<std::string>{a0, a1},
                   std::vector<std::string>{"label=\"$1\"", "label=\"$2\""});
    ...
    virtual std::string printAtomicForwardOp(OperationNode<Base>& atomicFor) {
        ...
        std::string name =
                printNodeDeclaration(atomicFor, _info->atomicFunctionId2Name.at(id) + ".forward(" + std::to_string(q) +
                                                        ", " + std::to_string(p) + ", tx, ty)");
    ...
    virtual std::string printAtomicReverseOp(OperationNode<Base>& atomicRev) {
        ...
        std::string name = printNodeDeclaration(
                atomicRev, _info->atomicFunctionId2Name.at(id) + ".reverse(" + std::to_string(p) + ", tx, px, py)");
    ...
    virtual std::string printLoopStart(OperationNode<Base>& node) {
        ...
        if (lnode.getIterationCountNode() != nullptr) {
            ...
            printEdge(*lnode.getIterationCountNode(), name,
    ...
    virtual std::string printStartIf(OperationNode<Base>& node) {
        ...
        printEdges(name, node, std::vector<std::string>{}, std::vector<std::string>{"label=\"condition\""});
    ...
    virtual std::string printElse(OperationNode<Base>& node) {
        ...
        printEdges(name, node, std::vector<std::string>{}, std::vector<std::string>{"label=\"false\""});
