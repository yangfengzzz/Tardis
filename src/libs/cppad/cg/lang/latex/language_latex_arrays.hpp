...
void LanguageLatex<Base>::printArrayCreationOp(OperationNode<Base>& array) {
    ...
    for (size_t i = 0; i < argSize; i++) {
        ...
        if (newValue) {
            if (firstElement) {
                _code << _startAlgLine << _startEq << auxArrayName_ << _assignStr
                      << _nameGen->generateTemporaryArray(array, getVariableID(array)) << _endEq << _endAlgLine
            ...
            if (newI == i) {
                ...
                _code << _startAlgLine << _startEq << auxArrayName_ << "[" << i << "]" << _assignStr;
                print(args[i]);
...
void LanguageLatex<Base>::printSparseArrayCreationOp(OperationNode<Base>& array) {
    ...
    for (size_t i = 0; i < argSize; i++) {
        ...
        if (newValue) {
            if (firstElement) {
                _code << _startAlgLine << _startEq << auxArrayName_ << _assignStr
                      << _nameGen->generateTemporarySparseArray(array, getVariableID(array)) << _endEq << _endAlgLine
            ...
            if (newI == i) {
                ...
                _code << _startEq << _C_SPARSE_INDEX_ARRAY << "[";
                if (startPos != 0) _code << startPos << "+";
                _code << i << "]" << _assignStr << info[i + 1] << _endEq << _endAlgLine << _endline;
                ...
                for (size_t j = i; j < newI; j++) {
                    _code << _startAlgLine << _startEq << _C_SPARSE_INDEX_ARRAY << "[";
                    if (startPos != 0) _code << startPos << "+";
                    _code << j << "]" << _assignStr << info[j + 1] << _endEq << _endAlgLine << _endline;
                }
...
inline size_t LanguageLatex<Base>::printArrayCreationUsingLoop(size_t startPos,
    ...
    _indentationLevel++;
    _code << _startAlgLine << _startEq << auxArrayName_ << "[i]" << _assignStr << arrayAssign.str() << _endEq
          << _endAlgLine << _endline;
    _indentationLevel--;
...
template <class Base>
inline void LanguageLatex<Base>::printArrayStructInit(const std::string& dataArrayName, OperationNode<Base>& array) {
    /**
     * TODO: finish this
     */
    ...
    if (array.getOperationType() == CGOpCode::ArrayCreation) {
        ...
        _code << dataArrayName << ".size = " << size << "; " << dataArrayName << ".sparse = " << false << ";";
    } else {
        // sparse array
        ...
        if (nnz > 0)
            ...
            _code << dataArrayName << ".data = NULL; ";
        _code << dataArrayName << ".size = " << array.getInfo()[0] << "; " << dataArrayName << ".sparse = " << true
