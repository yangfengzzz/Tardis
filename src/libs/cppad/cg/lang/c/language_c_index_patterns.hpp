...
template <class Base>
inline std::string LanguageC<Base>::indexPattern2String(const IndexPattern& ip, const OperationNode<Base>& index) {
    return indexPattern2String(ip, {index.getName()});
}

template <class Base>
inline std::string LanguageC<Base>::indexPattern2String(const IndexPattern& ip, const std::string& index) {
...
inline std::string LanguageC<Base>::indexPattern2String(const IndexPattern& ip,
    ...
    for (size_t i = 0; i < indexes.size(); ++i) indexStr[i] = indexes[i]->getName();
    ...
    switch (ip.getType()) {
        ...
        case IndexPatternType::Sectioned: {
            ...
            for (size_t s = 0; s < sSize - 1; s++) {
                ...
                ss << "(" << (*indexes[0]) << "<" << xStart << ")? " << indexPattern2String(*lp, *indexes[0]) << ": ";
            }
        ...
        case IndexPatternType::Plane2D:  // y = f(x) + f(z)
        {
            ...
            if (pip.getPattern1() != nullptr) indexExpr += indexPattern2String(*pip.getPattern1(), *indexes[0]);

            if (useParens) indexExpr += ") + (";

            if (pip.getPattern2() != nullptr) indexExpr += indexPattern2String(*pip.getPattern2(), *indexes.back());
...
template <class Base>
inline std::string LanguageC<Base>::linearIndexPattern2String(const LinearIndexPattern& lip, const std::string& index) {
...
template <class Base>
bool LanguageC<Base>::isOffsetBy(const IndexPattern* ip, const IndexPattern* refIp, long offset) {
    if (ip->getType() == IndexPatternType::Linear) {
        const auto* lIp = dynamic_cast<const LinearIndexPattern*>(ip);
        assert(lIp != nullptr);

        if (refIp->getType() != IndexPatternType::Linear) return false;
        ...
        if (refIp->getType() != IndexPatternType::Sectioned) return false;
...
template <class Base>
bool LanguageC<Base>::isOffsetBy(const LinearIndexPattern* lIp, const LinearIndexPattern* refLIp, long offset) {
    if (lIp == nullptr || refLIp == nullptr) return false;  // different pattern type
...
template <class Base>
bool LanguageC<Base>::isOffsetBy(const LinearIndexPattern& lIp, const LinearIndexPattern& refLIp, long offset) {
    return refLIp.getLinearSlopeDx() == lIp.getLinearSlopeDx() && refLIp.getLinearSlopeDy() == lIp.getLinearSlopeDy() &&
           refLIp.getXOffset() == lIp.getXOffset() &&
           refLIp.getLinearConstantTerm() + offset == lIp.getLinearConstantTerm();
}

template <class Base>
bool LanguageC<Base>::isOffsetBy(const SectionedIndexPattern* sIp, const SectionedIndexPattern* refSecp, long offset) {
    if (refSecp == nullptr || sIp == nullptr) return false;  // different pattern type
...
template <class Base>
bool LanguageC<Base>::isOffsetBy(const SectionedIndexPattern& sIp, const SectionedIndexPattern& refSecp, long offset) {
    if (refSecp.getLinearSections().size() != sIp.getLinearSections().size()) return false;  // different pattern type
...
template <class Base>
Plane2DIndexPattern* LanguageC<Base>::encapsulateIndexPattern(const LinearIndexPattern& refLIp, size_t starti) {
...
template <class Base>
Plane2DIndexPattern* LanguageC<Base>::encapsulateIndexPattern(const SectionedIndexPattern& refSecp, size_t starti) {
    std::unique_ptr<IndexPattern> ip2;
