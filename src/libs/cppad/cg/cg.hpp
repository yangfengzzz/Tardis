...
protected:
    ...
    inline void makeVariable(OperationNode<Base>& operation, std::unique_ptr<Base>& value);
...
template <class Base>
inline std::ostringstream& operator<<(std::ostringstream& os,  //< steam to write the vector to
                                      const CG<Base>& v        //< vector that is output
) {
    if (v.isParameter()) {
        os << v.getValue();
...
template <class Base>
inline std::istream& operator>>(std::istream& is,  //< stream to load a parameter value
