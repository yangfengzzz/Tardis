...
public:
    ...
    inline const IndexPattern& getIndexPattern() const { return indexPattern_; }

    inline IndexPattern& getIndexPattern() { return indexPattern_; }
...
protected:
    ...
    inline IndexAssignOperationNode(CodeHandler<Base>* handler,
        ...
        : OperationNode<Base>(handler, CGOpCode::IndexAssign, {index, index1}), indexPattern_(indexPattern) {}
    ...
    inline IndexAssignOperationNode(CodeHandler<Base>* handler,
        ...
        : OperationNode<Base>(
                  handler, CGOpCode::IndexAssign, std::vector<size_t>(0), createArguments(index, index1, index2)),
    ...
    inline static std::vector<Argument<Base>> createArguments(OperationNode<Base>& index,
        ...
        args[0] = index;
        if (index1 != nullptr) args[1] = *index1;
        if (index2 != nullptr) {
            args.back() = *index2;
        }
