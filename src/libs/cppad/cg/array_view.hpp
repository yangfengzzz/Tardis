...
public:
     ...
     */
    inline ArrayView() : _data(nullptr), _length(0) {}
     ...
     */
    inline ArrayView(pointer array, size_type n) : _data(array), _length(n) {
     ...
     */
    inline ArrayView(std::vector<value_type>& vector) : _data(vector.data()), _length(vector.size()) {}
     ...
     * @param vector the vector to wrap
     */
    inline ArrayView(CppAD::vector<value_type>& vector) : _data(vector.data()), _length(vector.size()) {}
    ...
    inline ArrayView(std::array<value_type, S>& array) : _data(array.data()), _length(S) {}
    ...
    inline ArrayView(std::valarray<value_type>& array)
        : _data(array.size() > 0 ? &array[0] : nullptr), _length(array.size()) {}
    ...
    inline ArrayView(const ArrayView<typename std::remove_const<value_type>::type>& array,
        ...
        : _data(array.data()), _length(array.size()) {}
    ...
    inline ArrayView(const std::vector<typename std::remove_const<value_type>::type>& vector,
                     typename std::enable_if<std::is_const<TT>::value>::type* = 0)
        : _data(vector.data()), _length(vector.size()) {}
     ...
     * @return number of elements in the array.
     */
    inline size_type max_size() const noexcept { return _length; }
     ...
     * @return an ArrayView encapsulating n elements starting at position start
     */
    inline ArrayView<const value_type> segment(size_t start, size_t n) const {
        CPPADCG_ASSERT_KNOWN(start < size(), "ArrayView::segment() start index must be lower than the array size");
        CPPADCG_ASSERT_KNOWN(start + n <= size(),
                             "ArrayView::segment() the new segment will end after the end of this array");
...
public:
    inline ArrayView& operator=(const ArrayView& x) {
        ...
        if (x.size() != size())
            throw CGException(
                    "ArrayView: assigning an array with different size: the left hand side array has the size ", size(),
    ...
    template <typename TT = const Type,
              typename = typename std::enable_if<!std::is_same<Type, TT>::value &&
                                                 std::is_assignable<Type&, TT&>::value>::type>
 ...
 */
template <class Type>
inline std::ostream& operator<<(std::ostream& os, const ArrayView<Type>& array) {
    size_t i = 0;
    size_t n = array.size();
