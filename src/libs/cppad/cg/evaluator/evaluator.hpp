...
public:
    ...
    inline virtual ~EvaluatorBase() { clear(); }

    /**
     * @return true if this Evaluator is currently being used.
     */
    inline bool isUnderEvaluation() { return underEval_; }
     ...
     */
    inline std::vector<ActiveOut> evaluate(ArrayView<const ActiveOut> indepNew, ArrayView<const CG<ScalarIn>> depOld) {
    ...
    inline void evaluate(const ActiveOut* indepNew,
        ...
        if (handler_.getIndependentVariableSize() != indepSize) {
            throw CGException("Invalid independent variable size. Expected ", handler_.getIndependentVariableSize(),
                              " but got ", indepSize, ".");
        }

        CPPADCG_ASSERT_KNOWN(handler_.getIndependentVariableSize() == indepSize,
                             "Invalid size the array of independent variables")
...
protected:
    ...
    inline ActiveOut evalArg(const std::vector<Argument<ScalarIn>>& args, size_t pos) {
        return evalArg(args[pos], pos);
    }

    inline ActiveOut evalArg(const Argument<ScalarIn>& arg, size_t pos) {
    ...
    inline const ActiveOut& evalOperations(OperationNode<ScalarIn>& node) {
        CPPADCG_ASSERT_KNOWN(node.getHandlerPosition() < handler_.getManagedNodesCount(),
    ...
    inline ActiveOut* saveEvaluation(const OperationNode<ScalarIn>& node, ActiveOut&& result) {
...
public:
    inline EvaluatorOperations(CodeHandler<ScalarIn>& handler) : Base(handler) {}
...
protected:
    ...
    inline void processActiveOut(const NodeIn& node, ActiveOut& a) {}
};
...
public:
    inline Evaluator(CodeHandler<ScalarIn>& handler) : Base(handler) {}
