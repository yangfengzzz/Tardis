...
public:
    inline CodeHandlerVectorSync(CodeHandler<Base>& handler) : handler_(&handler) { handler_->addVector(this); }

    inline CodeHandlerVectorSync(const CodeHandlerVectorSync& orig) : handler_(orig.handler_) {
        handler_->addVector(this);
    }

    virtual ~CodeHandlerVectorSync() {
        if (handler_ != nullptr) handler_->removeVector(this);
    }

    inline CodeHandler<Base>& getHandler() const { return *handler_; }
...
public:
    inline CodeHandlerVector(CodeHandler<Base>& handler) : CodeHandlerVectorSync<Base>(handler) {}

    inline CodeHandlerVector(const CodeHandlerVector& orig) : CodeHandlerVectorSync<Base>(orig), data_(orig.data_) {}
    ...
    inline void set(const OperationNode<Base>& node, const T& val) {
        CPPADCG_ASSERT_UNKNOWN(node.getCodeHandler() == this->handler_)
    ...
    inline void fill(const T& v) { std::fill(data_.begin(), data_.end(), v); }
    ...
    inline const_reverse_iterator rbegin() const { return data_.rbegin(); }

    inline reverse_iterator rend() { return data_.rend(); }

    inline const_reverse_iterator rend() const { return data_.rend(); }
    ...
    inline const_reverse_iterator crbegin() const noexcept { return data_.crbegin(); }

    inline const_reverse_iterator crend() const noexcept { return data_.crend(); }
...
public:
     ...
     * operators
     */
    inline reference operator[](const OperationNode<Base>& node) { return get(node); }

    inline const_reference operator[](const OperationNode<Base>& node) const { return get(node); }
};
