...
inline VectorBool jacobianForwardSparsity(ADFun<Base>& fun) {
    ...
    for (size_t j = 0; j < n; j++) {
        for (size_t k = 0; k < n; k++) r[j * n + k] = false;
        r[j * n + j] = true;
    }
...
inline VectorBool jacobianReverseSparsity(ADFun<Base>& fun) {
    ...
    for (size_t i = 0; i < m; i++) {
        for (size_t k = 0; k < m; k++) s[i * m + k] = false;
        s[i * m + i] = true;
    }
...
inline VectorSet jacobianReverseSparsitySet(ADFun<Base>& fun) {
    ...
    VectorSet s_s(m);
    for (size_t i = 0; i < m; i++) s_s[i].insert(i);
 ...
 */
inline bool estimateBestJacobianADMode(const std::vector<size_t>& jacRows, const std::vector<size_t>& jacCols) {
...
inline VectorBool hessianSparsity(ADFun<Base>& fun, size_t i, bool transpose = false) {
    ...
    VectorBool s(m);
    for (size_t ii = 0; ii < m; ii++) s[ii] = false;
    s[i] = true;
    return fun.RevSparseHes(n, s, transpose);
}
...
inline void generateSparsityIndexes(const VectorBool& sparsity, size_t m, size_t n, VectorSize& row, VectorSize& col) {
...
inline void generateSparsityIndexes(const VectorSet& sparsity, VectorSize& row, VectorSize& col) {
...
template <class VectorSet, class VectorSize>
inline void generateSparsitySet(const VectorSize& row, const VectorSize& col, VectorSet& sparsity) {
