...
inline CG<Base> CodeHandler<Base>::collectVariable(OperationNode<Base>& expression,
    ...
    for (size_t i = 0; i < indep.size(); ++i) indep[i] = CG<Base>(*this->_independentVariables[i]);
    ...
    e.evaluate(indep.data(), indep.size(), &expression2, &expressionOrig, 1);
...
inline CG<Base> CodeHandler<Base>::collectVariableAddSub(const SourceCodePath& pathLeft,
    ...
    if (initCommonOp != CGOpCode::Add && initCommonOp != CGOpCode::Sub) {
    ...
    for (size_t j = 0; j < paths.size(); ++j) {
        ...
        for (size_t i1 = p.size() - 1; i1 > 0; --i1) {
            size_t i = i1 - 1;
            if (p[i].node->getOperationType() == CGOpCode::Mul || p[i].node->getOperationType() == CGOpCode::UnMinus ||
                p[i].node->getOperationType() == CGOpCode::Div || p[i].node->getOperationType() == CGOpCode::Alias) {
                replace[j][i] = &zero;
            } else {
                break;
            }
...
inline bool CodeHandler<Base>::isCollectableVariableAddSub(const SourceCodePath& pathLeft,
    ...
    for (const SourceCodePath* p : paths) {
        for (size_t i = 1; i < p->size() - 1; ++i) {  // the last node does not have to be +, -, or *
            ...
            auto op = node->getOperationType();
            if (op != CGOpCode::Add && op != CGOpCode::Sub && op != CGOpCode::Mul && op != CGOpCode::UnMinus &&
                op != CGOpCode::Alias) {
                if (op == CGOpCode::Div) {
                    if ((*p)[i].argIndex != 0) {
                        if (throwEx)
                            throw CGException(
                                    "Unable to combine operations which are present in denominators (not implemented "
                                    "yet).");
                        return false;
                    } else {
                        continue;
                ...
                if (throwEx)
                    throw CGException(
                            ...
                            " Unable to combine operations diverging at a '",
                            op, "' operation.");
                return false;
            }
        }
